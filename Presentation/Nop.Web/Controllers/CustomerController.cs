...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BasePublicController
    {
        ...
        private readonly IHistoryMoneyService _historyMoneyService;
        ...
        public CustomerController(IAuthenticationService authenticationService,
            ...
            IHistoryMoneyService historyMoneyService,
            ...
            VendorSettings vendorSettings)
        {
            ...
            this._historyMoneyService = historyMoneyService;
        ...
        public ActionResult CustomerNavigation(int selectedTabId = 0)
        {
            ...
            model.CustomerNavigationItems.Add(new CustomerNavigationItemModel
            {
                RouteName = "CustomerNapthe",
                Title = _localizationService.GetResource("Account.CustomerNapThe"),
                Tab = CustomerNavigationEnum.NapThe,
                ItemClass = "customer-napthe"
            });
            //model.CustomerNavigationItems.Add(new CustomerNavigationItemModel
            //{
        ...
        [HttpPost]
        public ActionResult NapThe(NapTheModel model)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri("https://sv.gamebank.vn/api/card");
                ApiNapTheModel api = new ApiNapTheModel
                {
                    merchant_id = 49464,
                    api_user = "5a68485379f4a",
                    api_password = "",
                    pin = model.pin,
                    seri = model.seri,
                    price_guest = model.price_guest,
                    card_type = model.card_type_id,
                    note = ""
                };
                var response = client.PostAsJsonAsync("https://sv.gamebank.vn/api/card", api).Result;
                if (response.IsSuccessStatusCode)
                {
                    var jsonString = response.Content.ReadAsStringAsync();
                    jsonString.Wait();
                    var resultApi = JsonConvert.DeserializeObject<ResultApiModel>(jsonString.Result);
                    if (resultApi.code == 0)
                    {
                        HistoryMoney historymoney = new HistoryMoney();
                        historymoney.CustomerId = _workContext.CurrentCustomer.Id;
                        historymoney.Pin = model.pin;
                        historymoney.Seri = model.seri;
                        historymoney.CreatedOnUtc = DateTime.Now;
                        historymoney.Status = true;
                        var customer = _customerService.GetCustomerById(_workContext.CurrentCustomer.Id);
                        customer.Money = customer.Money + resultApi.info_card;
                        _customerService.UpdateCustomer(customer);
                        _historyMoneyService.InsertHistoryMoney(historymoney);
                    }
                    else
                    {
                        HistoryMoney historymoney = new HistoryMoney();
